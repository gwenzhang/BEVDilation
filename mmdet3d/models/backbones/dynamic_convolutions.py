...
from mmdet3d.models.backbones.common import TempModule
...
class AttentionLayer_Multiple(nn.Module):
    def __init__(self, c_dim, hidden_dim, nof_kernels):
        super().__init__()
        self.global_pooling = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten())
        self.to_scores = nn.Sequential(nn.Linear(c_dim, hidden_dim),
        ...
        self.proj_img = nn.Sequential(nn.Conv2d(32, hidden_dim, kernel_size=3, stride=1, padding=1),
                                    nn.BatchNorm2d(hidden_dim),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(hidden_dim, c_dim, kernel_size=3, padding=1))

    def forward(self, x, y, temperature=1):
        y = self.proj_img(y)
        out = self.global_pooling(x) + self.global_pooling(y)
        scores = self.to_scores(out)
        return F.softmax(scores / temperature, dim=-1)
...
class DynamicConvolution(TempModule):
    def __init__(self, nof_kernels, reduce, in_channels, out_channels, kernel_size,
        ...
        # self.attention = AttentionLayer_Multiple(in_channels, max(1, in_channels // reduce), nof_kernels)
