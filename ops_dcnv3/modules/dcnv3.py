...
class DCNv3(nn.Module):
    def __init__(
        ...
        self.input_proj = nn.Linear(channels, channels)
        self.output_proj = nn.Linear(channels, channels)
        # additional parameters for multi-modaliates
        self.use_img = use_img
        if use_img:
            self.fusion = nn.Sequential(
                nn.Conv2d(channels + 32, channels, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, input, img_input=None):
        """
        :param query                       (N, H, W, C)
        :return output                     (N, H, W, C)
        """
        ...
        x1 = input.permute(0, 3, 1, 2)
        x1 = self.fusion(torch.cat([x1, img_input], dim=1)) if self.use_img else x1
        x1 = self.dw_conv(x1)
        offset = self.offset(x1)
        mask = self.mask(x1).reshape(N, H, W, self.group, -1)
